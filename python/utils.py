...
:Edits made by: Ilina Bhaya-Grossman (2025)
"""
import pickle
import numpy as np
...
def load_data(participant_num=1, sentence_num=1):
    """
    Load the data for a given participant and sentence.

    Parameters
    ----------
    participant_num : int
        Which participant's brain activity to load. Defaults to `1`, can be
        `1` or `2`.
    sentence_num : int
    ...
    Returns
    -------
    A dictionary containing the data for the specified participant and
    sentence.
    """
    with open(f'{DATA_PATH}/P{participant_num}_data.pkl', 'rb') as f:
        data = pickle.load(f)
    return data[f'sentence{sentence_num}']
...
def get_sentence_text(sent_num):
    ...
    """
    with open('{DATA_PATH}/P1_data.pkl', 'rb') as f:
        data = pickle.load(f)
...
def play_brain_movie(sentence_number=1, speed='normal', filename=None):
    ...
    movie_path = f'{MEDIA_PATH}/{filename}'
